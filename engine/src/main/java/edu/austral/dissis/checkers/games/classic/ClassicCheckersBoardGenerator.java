...
public class ClassicCheckersBoardGenerator implements BoardGenerator {
  ...
  private void generateWhiteCheckers(
      PieceFactory pieceGenerator, HashMap<Coordinates, Piece> pieces) {
    for (int x = 1; x <= 8; x++) {
      for (int y = 1; y <= 3; y++) {
        if ((x + y) % 2 == 0) {
          pieces.put(
              new Coordinates(x, y),
              pieceGenerator.generatePiece(CheckersPieceNames.PAWN, PieceColor.WHITE));
        }
      }
    }
  }

  private void generateBlackCheckers(
      PieceFactory pieceGenerator, HashMap<Coordinates, Piece> pieces) {
    for (int x = 1; x <= 8; x++) {
      for (int y = 6; y <= 8; y++) {
        if ((x + y) % 2 == 0) {
          pieces.put(
              new Coordinates(x, y),
              pieceGenerator.generatePiece(CheckersPieceNames.PAWN, PieceColor.BLACK));
        }
      }
    }
  }
