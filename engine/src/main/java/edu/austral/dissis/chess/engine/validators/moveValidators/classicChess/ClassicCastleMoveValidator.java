...
public class ClassicCastleMoveValidator implements MoveValidator {
    ...
    public boolean validMove(Coordinates from, Coordinates to, Board board) {
        ...
        if (to.getX() == 7 && to.getY() == row)
            return canCastleKingSide(from, to, board);
        else if (to.getX() == 3 && to.getY() == row)
            return canCastleQueenSide(from, to, board);
    ...
    private boolean canCastleKingSide(Coordinates from, Coordinates to, Board board){
        ...
        if (board.isSquareThreatened(new Coordinates(6, to.getY())))
            return false;
    ...
    private boolean canCastleQueenSide(Coordinates from, Coordinates to, Board board){
        ...
        if (board.isSquareThreatened(new Coordinates(4, to.getY())))
            return false;
    ...
    private boolean canCastle(Coordinates from, Coordinates to, Piece kingRook, Board board) {
        ...
        if (!kingRook.isFirstMove() && !king.isFirstMove())
            return false;
        if(!clearPathValidator.validMove(from, to, board))
            return false;
